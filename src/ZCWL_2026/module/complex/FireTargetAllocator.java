...
public class FireTargetAllocator extends adf.core.component.module.complex.FireTargetAllocator {
  ...
  public FireTargetAllocator updateInfo( MessageManager messageManager ) {
    ...
    // Process fire brigade specific messages
    ...
    for ( CommunicationMessage message : messageManager.getReceivedMessageList( CommandFire.class ) ) {
      ...
      if ( command.getAction() == CommandFire.ACTION_RESCUE && command.isBroadcast() ) {
  ...
  /**
   * Convert FireBrigadeInfo map to EntityID mapping
   */
  ...
  /**
   * Get available fire brigade agents
   */
  ...
  /**
   * Update fire brigade info based on received message
   */
  ...
  /**
   * Fire brigade status information class
   */
  ...
  /**
   * Distance comparator for sorting entities by distance to a reference
   */
