...
public class ActionExtClear extends adf.core.component.extaction.ExtAction {
    ...
    switch ( si.getMode() ) {
      case PRECOMPUTATION_PHASE:
        this.pathPlanning = moduleManager.getModule(
          ...
          "ZCWL_2026.module.algorithm.SamplePathPlanning" );
        break;
      case PRECOMPUTED:
        this.pathPlanning = moduleManager.getModule(
            "ActionExtClear.PathPlanning",
  ...
  public ExtAction setTarget( EntityID target ) {
    ...
    if ( entity != null ) {
      if ( entity instanceof Road ) {
        this.target = target;
      } else if ( entity.getStandardURN()
          .equals( StandardEntityURN.BLOCKADE ) ) {
  ...
  private Action getRescueAction( PoliceForce police, Road road ) {
    ...
    Collection<Blockade> blockades = this.worldInfo.getBlockades( road )
        .stream().filter( Blockade::isApexesDefined )
    ...
    for ( StandardEntity entity : agents ) {
      ...
      for ( Blockade blockade : blockades ) {
        ...
        if ( this.intersect( policeX, policeY, humanX, humanY, road ) ) {
          ...
          if ( action.getClass() == ActionClear.class ) {
            ...
            if ( actionClear.getTarget() != null ) {
              ...
              int anotherDistance = this.worldInfo.getDistance( police,
                  another );
        ...
        } else if ( this.intersect( policeX, policeY, humanX, humanY,
            blockade ) ) {
  ...
  private Action getIntersectEdgeAction( double agentX, double agentY,
    ...
    if ( bestPoint != null ) {
      ...
      ActionMove actionMove = null;
      Vector2D vector = this
          .scaleClear( this.getVector( agentX, agentY, pX, pY ) );
      int clearX = (int) ( agentX + vector.getX() );
      int clearY = (int) ( agentY + vector.getY() );
      vector = this.scaleBackClear( vector );
      int startX = (int) ( agentX + vector.getX() );
  ...
  private boolean equalsPoint( double p1X, double p1Y, double p2X,
      double p2Y ) {
  ...
  private boolean isInside( double pX, double pY, int[] apex ) {
    ...
    Vector2D v1 = ( new Point2D( apex[apex.length - 2],
        apex[apex.length - 1] ) ).minus( p );
