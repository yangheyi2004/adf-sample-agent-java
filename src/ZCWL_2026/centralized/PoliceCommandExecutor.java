...
public class PoliceCommandExecutor extends adf.core.component.centralized.CommandExecutor {
    ...
    public PoliceCommandExecutor(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
        super(ai, wi, si, moduleManager, developData);
        this.commandType = ACTION_UNKNOWN;

        switch  (scenarioInfo.getMode()) {
            case PRECOMPUTATION_PHASE:
                this.pathPlanning = moduleManager.getModule("PoliceCommandExecutor.PathPlanning", "ZCWL_2026.module.algorithm.PathPlanning");
                this.actionExtClear = moduleManager.getExtAction("PoliceCommandExecutor.ActionExtClear", "ZCWL_2026.extraction.ActionExtClear");
                this.actionExtMove = moduleManager.getExtAction("PoliceCommandExecutor.ActionExtMove", "ZCWL_2026.extraction.ActionExtMove");
                break;
            case PRECOMPUTED:
                this.pathPlanning = moduleManager.getModule("PoliceCommandExecutor.PathPlanning", "ZCWL_2026.module.algorithm.PathPlanning");
                this.actionExtClear = moduleManager.getExtAction("PoliceCommandExecutor.ActionExtClear", "ZCWL_2026.extraction.ActionExtClear");
...
public CommandExecutor setCommand(CommunicationMessage command) {
    if (command instanceof CommandPolice) {
        CommandPolice cmd = (CommandPolice) command;
                ...
                if(targetEntity.getStandardURN() == REFUGE) {
                    ...
                    if(agent.getDamage() > 0) {
                        if (position.getValue() != this.target.getValue()) {
                            List<EntityID> path = this.pathPlanning.getResult(position, this.target);
                            if(path != null && path.size() > 0) {
                                Action action = this.actionExtMove.setTarget(path.get(path.size() - 1)).calc().getAction();
                                if (action == null) {
                                    action = new ActionMove(path);
                                }
                                this.result = action;
